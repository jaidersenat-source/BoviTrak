<x-app-layout>
    <x-slot name="title">Nuevo Registro de Descendencia – {{ $animal->nombre ?? $animal->codigo_nfc }}</x-slot>
    <x-slot name="header">
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
            ...
            <div class="p-6">
                @include('admin.descendencia.form', [
                    'animal' => $animal,
                    'record' => new \App\Models\AnimalDescendance(),
                    'padres' => $padres ?? collect(),
                    'madres' => $madres ?? collect(),
                    'action' => route('animals.descendencia.store', $animal),
                    'method' => 'POST',
                ])
            </div>
        </div>
