<x-app-layout>
    <x-slot name="title">Nuevo Proceso Reproductivo – {{ $animal->nombre ?? $animal->codigo_nfc }}</x-slot>
    <x-slot name="header">
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
            <div class="card-header-green">
                <h3 class="text-xl font-bold text-white flex items-center gap-2">
                    ...
                    Registro de Proceso Reproductivo
                </h3>
            </div>
            <div class="p-6">
                @include('admin.reproductivo.form', [
                    'animal'       => $animal,
                    'record'       => new \App\Models\AnimalReproductiveRecord(),
                    'tiposProceso' => $tiposProceso,
                    'action'       => route('animals.reproductive.store', $animal),
                    'method'       => 'POST',
                ])
            </div>
        </div>
